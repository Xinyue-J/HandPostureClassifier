...
print('acc_train:', clf.score(data.iloc[:, 2:], data.iloc[:, 0]))
pred_test_label = pd.DataFrame(clf.predict(test.iloc[:, 2:]))
print('acc_test:', accuracy_score(pred_test_label, test.iloc[:, 0]))
print('acc_test:', clf.score(test.iloc[:, 2:], test.iloc[:, 0]))

# confusion matrix

C = confusion_matrix(test.iloc[:, 0], pred_test_label, labels=[1, 2, 3, 4, 5])
print(C)
